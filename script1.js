$(document).ready(function(){
    ...
    function playGame(){
        ...
        for(var i = 0; i < sentences.length; i++){
            var sentence = sentences[i];//working with 1 sentence at a time
            $('#sentence').append(sentence);

            var arrayLettersToType = sentenceLetters(sentence);//array with each letter in the sentence as an element

            for(var j = 0; j < arrayLettersToType.length; j++){
                var letterToType = arrayLettersToType[j];//working with 1 letter at a time
                $('#target-letter').append(letterToType);
                letterToType = letterToType.charCodeAt(0);
                console.log('letterToType: ' + letterToType);
                var pressedLetter = 0;
                ...
                if(letterToType == pressedLetter){
                    $('#feedback').append('<div class = "correctKey">&#9989</div>');
                    $('#feedback').css('position', 'relative');
                }
                else{
                    $('#feedback').append('<div class = "wrongKey">&#10062</div>');
                    $('#feedback').css('position', 'relative');
                };
